...
Dérivé de jeedom-audit/scenario_tree_walker.py — db_query.py → _core/db.py (PyMySQL local).
"""
...
log = structlog.get_logger('holmesMcp.domain.scenario_walker')

DEFAULT_MAX_DEPTH = 3
MAX_SUB_ELEMENTS = 100
...
def _fetch_scenario(scenario_id: int, conn) -> dict | None:
    rows = db.query(conn, _SCENARIO_SQL, (scenario_id,))
    return rows[0] if rows else None


def _fetch_elements(element_ids: list[int], conn) -> list[dict]:
    if not element_ids:
        return []
    placeholders = ', '.join(['%s'] * len(element_ids))
    sql = _ELEMENTS_SQL.format(placeholders=placeholders)
    return db.query(conn, sql, tuple(element_ids))
...
def _child_element_ids(expressions: list[dict]) -> list[int]:
    """Extrait les IDs des sous-éléments référencés par type='element'."""
    children = []
    for expr in expressions:
        if expr.get('type') == 'element':
            try:
                children.append(int(expr['expression']))
...
def _extract_scenario_call_id(expr: dict) -> int | None:
    ...
    Seul action='start' (ou None/'') est suivi.
    """
...
def _walk(
    ...
    rows = _fetch_elements(safe_ids, conn)
    ...
    for el_id in safe_ids:
        ...
        if len(sub_elements) > MAX_SUB_ELEMENTS:
            warnings.append(
                f'element {el_id} : {len(sub_elements)} sous-éléments'
                f' — tronqué à {MAX_SUB_ELEMENTS}'
            )
            sub_elements = sub_elements[:MAX_SUB_ELEMENTS]
            truncated_flag[0] = True

        all_expr = [expr for sub in sub_elements for expr in sub['expressions']]
        child_ids = _child_element_ids(all_expr)
...
def walk(
    ...
    """
    log.debug('scenario_walk', scenario_id=scenario_id, max_depth=max_depth)
    ...
    scenario = _fetch_scenario(scenario_id, conn)
    if scenario is None:
        return {
